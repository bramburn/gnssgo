...
func TestServerCasterIntegration(t *testing.T) {
	...
	dataSource.dataChan <- []byte("test data")
	...
	if err != nil {
		t.Logf("Error connecting to caster: %v", err)
		t.FailNow()
	}
	...
	if err != nil && err != io.EOF {
		t.Logf("Error reading from caster: %v", err)
		t.FailNow()
	}

	// For NTRIP v1, the response starts with "ICY 200 OK\r\n"
	if n > 0 {
		// Skip the header for NTRIP v1
		if string(buffer[:10]) == "ICY 200 OK" {
			...
			n, err = resp.Body.Read(buffer)
			if err != nil && err != io.EOF {
				t.Logf("Error reading data from caster: %v", err)
				t.FailNow()
			}
		}
