...
type GNSSSerialPort struct {
	port   serial.Port
	config SerialConfig
	name   string // Store the port name for reconnection
}
...
func (p *GNSSSerialPort) Open(portName string, baudRate int) error {
	...
	p.port = port
	p.name = portName // Store the port name for reconnection
...
func (p *GNSSSerialPort) ChangeBaudRate(baudRate int) error {
	...
	portName := p.name
	if portName == "" {
		return fmt.Errorf("port name not available for reconnection")
	}
